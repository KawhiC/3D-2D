...
@torch.no_grad()
def medsam_inference(medsam_model, img_embed, box_256=None, new_size=(256, 256), original_size=(256, 256)):
    if box_256 is None:
        box_256 = np.array([[0, 0, new_size[1], new_size[0]]])  # 全图作为边界框
...
def MedSAM_infer_npz(gt_path_file):
    ...
    if (not isfile(join(pred_save_dir, task_folder, npz_name))) or overwrite:
        ...
        #img_256_padded = np.expand_dims(img_256_padded, axis=2)
        img_256_tensor = torch.tensor(img_256_padded).float().permute(2, 0, 1).unsqueeze(0).to(device)
        with torch.no_grad():
            image_embedding = medsam_lite_model.image_encoder(img_256_tensor)


        sam_mask = medsam_inference(medsam_lite_model, image_embedding, box_256=None, new_size=(newh, neww), original_size=(H, W))
        segs[sam_mask>0] = 1
        ...
        if save_overlay:
            ...
            show_mask(segs.astype(np.uint8), ax[1], mask_color=color)
            plt.tight_layout()
