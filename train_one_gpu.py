...
for epoch in range(start_epoch + 1, num_epochs):
    ...
    torch.save(checkpoint, join(work_dir, "medsam_lite_latest_withoutLiteMedSAM.pth"))
    if epoch_loss_reduced < best_loss:
        print(f"New best loss: {best_loss:.4f} -> {epoch_loss_reduced:.4f}")
        best_loss = epoch_loss_reduced
        checkpoint["best_loss"] = best_loss
        torch.save(checkpoint, join(work_dir, "medsam_lite_best_withoutLiteMedSAM.pth"))
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.savefig(join(work_dir, "train_loss_withoutLiteMedSAM.png"))
    plt.close()
